// vision.rs
//
// Generates a structured world state prompt suitable for an LLM to interpret.
// Includes player stats, nearby NPCs/players, chat log, and combat log.

use crate::world::world_state::WorldState;
use std::fmt::Write;

/// Generate a structured string representation of the world state for the LLM.
///
/// # Arguments
/// * `world` - Shared world state
/// * `self_guid` - The bot's own player GUID
///
/// # Returns
/// A formatted prompt string suitable for the LLM to parse and respond to.
pub fn generate_prompt(world: &WorldState, self_guid: u64) -> String {
    ...
    writeln!(
        prompt,
        "- ChatLog: {}",
        if world.chat_log.is_empty() { "empty" } else { "visible" }
    )
    .unwrap();
